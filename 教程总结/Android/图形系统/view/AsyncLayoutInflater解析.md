...
不管inflate成功或失败后，都会将request消息发送给主线程做处理   //todo android提供的对象池SynchronizedPool
```
...
1 所有构建的View中必须不能直接使用 Handler 或者是调用 Looper.myLooper()，因为异步线程默认没有调用 Looper.prepare ()    说的是handler是主线程的吧
...
直接上代码    //todo LayoutInflaterCompat  AppCompatDelegate
```
