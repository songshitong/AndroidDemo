...
int BFS(Node start, Node target) {
    Queue<Node> q; // 核心数据结构
    Set<Node> visited; // 避免走回头路  二叉树没有关系，图的情况需要该结构
    ...
    while (q not empty) {
        ...
        /* 将当前队列中的所有节点向四周扩散 */     注意每次取的是一层的节点，遍历这一层的每个节点
        for (int i = 0; i < sz; i++) {
            Node cur = q.poll();
            /* 划重点：这里判断是否到达终点 */     直接退出就行，不用break判断
            if (cur is target)
                return step;
        ...
        /* 划重点：更新步数在这里 */   这里增加步数，是将每一层都添加到队列时增加
        step++;
    }
    return step;
}
...
还是拿刚才我们处理二叉树问题的例子，假设给你的这个二叉树是满二叉树，节点数为 N，对于 DFS 算法来说，空间复杂度无非就是递归堆栈，
  最坏情况下顶多就是树的高度，也就是 O(logN)。  //二叉树高度为logN
